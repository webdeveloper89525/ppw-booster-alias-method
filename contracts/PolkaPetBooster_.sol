...
contract PolkaPetBooster is VRFConsumerBase, Ownable, ERC1155Holder, ReentrancyGuard, Pausable {
	...
	function fulfillOpenPack(bytes32 requestId, uint256 randomness) internal {
		...
		uint256[] memory ids = new uint256[](numSlots);
		uint256[] memory randoms;
		uint256[] memory amounts = new uint256[](numSlots);

		randoms = expand(randomness, numSlots);

		for(uint256 i = 0; i < numSlots; i++){
			uint256 random = randoms[i];
			bool found = false;
			uint256 iterations;
			while(! found){
				uint256 n = slots[i].index.length;
				uint256 x = randoms[i] % ONE_FIXED_POINT;
				uint256 nx = x * n;
				uint256 selectedSlotIndex = nx >> 236;
				uint256 y = nx & (type(uint256).max >> 20);
				...
				if(y < slotIndex.prob){
					...
					if(nftQty > 10){
						...
						iterations++;
						random = expandOnce(random, iterations);
					}
